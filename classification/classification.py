...
save_log = 'log.txt'
model_weights = 'resnet101_v2/resnet101_v2.caffemodel'
model_deploy = 'resnet101_v2/deploy_resnet101_v2.prototxt'
prob_layer = 'prob'
class_num = 1000
base_size = 256
crop_size = 224
...
def eval_batch():
    ...
    for i in xrange(eval_len - skip_num):
        _img = cv2.imread(data_root + eval_images[i + skip_num]) / raw_scale
    ...
    print '\n{} images has been tested, crop_num is: {}. \nThe model is: {}'.format(str(eval_len), str(crop_num), model_weights)
    for i in xrange(len(top_k)):
        _acc = float(accuracy[i]) / float(eval_len)
        print 'Accuracy of top_{} is: {}; correct num is {}.'.format(str(top_k[i]), str(_acc), str(accuracy[i]))
